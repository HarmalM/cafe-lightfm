...
PHASE 3, STEP 7 ADDITION (2026-07-06, confirmed by PI):
    `per_user_metric()` gains two optional, default-None parameters --
    `stage_idx_override` and `stage_to_idx` -- passed straight through
    to `ranked_relevance_for_user()` (experiments/ndcg.py). Both default
    to None, which reproduces Step 1-6 behavior exactly.

    `run_ndcg_precision_paired_comparison()` is INTENTIONALLY LEFT
    UNCHANGED. It hardcodes is_cafe=True for its first model argument
    and is_cafe=False for its second (i.e., it assumes a CAFE-LightFM
    vs. stage-blind-LightFM-baseline comparison, which is correct for
    Step 5/6 and remains correct for a future Full-vs-LightFM-baseline
    comparison). It is NOT reused for Step 7's ablation-vs-ablation
    comparisons, because noStage/noAttention/2Stage are ALL CAFELightFM
    instances (is_cafe=True on both sides) -- forcing one of them
    through the is_cafe=False path would call it with the wrong
    argument signature (4 args instead of 5) and raise a TypeError.
    Step 7 instead defines its own
    `run_cafe_variant_paired_comparison()` in
    experiments/step7_ablation_study.py, which reuses `per_user_metric`,
    `align_paired_arrays`, `paired_t_test`, and
    `summarize_paired_results` from this module UNCHANGED.
...
def per_user_metric(
    ...
    stage_idx_override : Optional[int], default None
        Phase 3, Step 7 (noStage ablation). Passed straight through to
        `ranked_relevance_for_user`. Default None preserves Step 1-6
        behavior exactly. See experiments/ndcg.py for full rationale.
    stage_to_idx : Optional[Dict[str, int]], default None
        Phase 3, Step 7 (2Stage ablation). Passed straight through to
        `ranked_relevance_for_user`. Default None preserves Step 1-6
        behavior exactly.
...
def run_ndcg_precision_paired_comparison(
    ...
    UNCHANGED at Phase 3, Step 7 -- see module docstring's Step 7
    addition note for why this function is not reused for
    ablation-vs-ablation comparisons.
