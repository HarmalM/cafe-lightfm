...
--------------------------------------------------------------------------
EXPLICIT ASSUMPTION (labeled per project protocol)
--------------------------------------------------------------------------
To compute alpha for the 30 catalog items, this script needs each item's
(category_idx, program_idx). This mapping is NOT stored in the
checkpoint -- it lives in the InteractionMatrixBundle produced by
data/synthetic_generator_v3.py (the same object consumed by
warp_loss.gather_item_features(); see bundle.item_feature_idx_by_item
and bundle.n_items, both confirmed via warp_loss.py inspection).

ASSUMPTION: the v3 dataset module exposes a bundle-construction entry
point, referenced here as `build_synthetic_v3_bundle(seed=42)` from
`data.synthetic_generator_v3`. This exact function name was NOT directly
visible in the three source files inspected so far (cafe_lightfm.py,
sca_layer.py, warp_loss.py) -- only the InteractionMatrixBundle
attributes it must produce were confirmed. IMPACT if the name/signature
differs: only `load_bundle()` below needs a one-line edit; every
downstream step (alpha extraction, tables, JSD, plots) is unaffected.
Please confirm or correct the entry-point name before the Colab run in
Section "COLAB COMMANDS" below.
...
def load_bundle(seed: int = 42):
    ...
    item's (category_idx, program_idx). See module docstring's "EXPLICIT
    ASSUMPTION" section: the exact entry-point name below
    (`build_synthetic_v3_bundle`) is an assumption pending confirmation.
    """
    try:
        from data.synthetic_generator_v3 import build_synthetic_v3_bundle
    except ImportError as exc:
        raise ImportError(
            "Could not import `build_synthetic_v3_bundle` from "
            "data.synthetic_generator_v3. This entry-point name was an "
            "explicit, labeled assumption (see module docstring) because "
            "data/synthetic_generator_v3.py was not among the files "
            "inspected for Step 6. Please provide the correct function "
            "name/signature that returns an InteractionMatrixBundle with "
            "`.item_feature_idx_by_item` and `.n_items` attributes, or "
            "paste the file so this loader can be corrected precisely."
        ) from exc
    return build_synthetic_v3_bundle(seed=seed)
