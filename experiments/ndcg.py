...
PHASE 3, STEP 7 ADDITION (2026-07-06, confirmed by PI):
    `generate_user_ranking()` and `ranked_relevance_for_user()` gain two
    optional, default-None parameters:

        stage_idx_override : Optional[int] = None
            When not None, used as the model's stage_idx instead of
            resolving it from stage_key via a STAGE_TO_IDX mapping.
            Required for evaluating the "CAFE-LightFM-noStage" ablation
            (proposal Section 5.6) correctly: noStage was TRAINED with
            every stage's data scored under a single fixed stage_idx
            (see experiments/training_loop.py's stage_idx_override), so
            w_stage[k] for k != stage_idx_override is never trained
            (remains at zero-initialization). Evaluating noStage on
            stage_key='S2'/'S3' via the ORDINARY STAGE_TO_IDX mapping
            would silently score with these untrained parameters,
            producing a train/eval mismatch that contaminates the
            ablation. This override lets evaluation use the SAME fixed
            stage_idx the model was actually trained under.

        stage_to_idx : Optional[Dict[str, int]] = None
            When not None, used instead of the module-level
            STAGE_TO_IDX. Required for the "CAFE-LightFM-2Stage"
            ablation, whose merged stage key ("decision") is not a
            member of the module-level STAGE_TO_IDX and would otherwise
            raise a KeyError.

    Both default to None, which reproduces Step 1-6 behavior exactly
    (byte-identical stage_idx resolution via the module-level
    STAGE_TO_IDX). `stage_wise_ndcg()` is UNCHANGED -- it always uses
    the ordinary 3-stage STAGE_ORDER/STAGE_TO_IDX and is not used for
    ablation evaluation (Step 7's ablation script calls
    generate_user_ranking/ranked_relevance_for_user, via
    experiments/step4_integration.py's per_user_metric, directly).
...
def generate_user_ranking(
    ...
    Exposed publicly for reuse by SW-NDCG (Phase 3, Step 2) and by the
    Step 4 integration layer / Step 7 ablation script.

    Parameters
    ----------
    user_idx   : integer index of the user (from bundle.user_id_to_idx)
    stage_key  : one of 'S1', 'S2', 'S3' (or a custom key, e.g.
                 'decision', if a custom `stage_to_idx` is supplied).
    ...
    stage_idx_override : Optional[int], default None
        Phase 3, Step 7 (noStage ablation). When not None, used directly
        as the model's stage_idx instead of resolving it from
        stage_key. Ignored when is_cafe is False. Default None preserves
        Step 1-6 behavior exactly.
    stage_to_idx : Optional[Dict[str, int]], default None
        Phase 3, Step 7 (2Stage ablation). When not None, used instead
        of the module-level STAGE_TO_IDX to resolve stage_key -> index.
        Ignored when is_cafe is False, or when stage_idx_override is
        set. Default None preserves Step 1-6 behavior exactly.
    ...
    with torch.no_grad():
        if is_cafe:
            ...
            stage_idx = (
                ...
                else effective_stage_to_idx[stage_key]
            )
...
def ranked_relevance_for_user(
    ...
    Parameters
    ----------
    stage_idx_override, stage_to_idx : see generate_user_ranking(). Both
        default to None, preserving Step 1-6 behavior exactly.
    """
    ranking = generate_user_ranking(
        user_idx, stage_key, model, bundle, is_cafe,
...
def stage_wise_ndcg(
    ...
    UNCHANGED from Phase 3, Step 1 -- always uses the ordinary 3-stage
    STAGE_ORDER/STAGE_TO_IDX. Not used for Step 7 ablation evaluation
    (see module docstring); Step 7 calls generate_user_ranking /
    ranked_relevance_for_user directly, via
    experiments/step4_integration.py's per_user_metric.
