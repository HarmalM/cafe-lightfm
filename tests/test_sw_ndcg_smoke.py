...
def test_weight_validation_rejects_negative_weight() -> None:
    """A weight dict can sum to exactly 1.0 while still containing a
    negative entry (e.g. 1.20 - 0.20 + 0.00 = 1.0), which would break the
    [0,1]-boundedness guarantee of SW-NDCG@K. Must be rejected
    independently of the sum-to-1 check."""
    stage_results = {"S1": {5: 0.5}, "S2": {5: 0.5}, "S3": {5: 0.5}}
    try:
        compute_sw_ndcg_from_stage_results(
            stage_results,
            weights={"S1": 1.20, "S2": -0.20, "S3": 0.00},
            k_values=(5,),
        )
    except ValueError as exc:
        print(f"[PASS] (2c) negative-weight validation raised as expected: {exc}")
        return
    raise AssertionError("Expected ValueError for negative stage weight")
...
if __name__ == "__main__":
    ...
    test_weight_validation_rejects_negative_weight()
